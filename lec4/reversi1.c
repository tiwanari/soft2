 ...
 * 氏名: 岩成達哉
 *  オセロAI
 *      課題1: 合法手がない場合はパスするように
 *           ll.122-143にgenerate_moves()という合法手の数をカウントする関数を作成し，
 *           ll.194-230の人間の入力を受け付けるman_player()という関数内で
 *          合法手の数が0のときはパスをするようにしました
 *           また，ll.256-258では両方ともパスの時は終了するようにしました
 *           あとは，表示の仕方の変更やコードの整理を行なっています
 */
